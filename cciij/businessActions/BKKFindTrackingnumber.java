...
public class BKKFindTrackingnumber extends BusinessActionBase {

    private static final String m_whatVersion = "@(#) $RCSfile: BKKFindTrackingnumber.java,v $ $Revision: 1.1 $ $Author: xinghai $ $Date: 2006/06/26 07:26:01 $\n";
    ...
    public BKKFindTrackingnumber() throws Exception {
...
//    state.getScan().setReturnMessage(state.getScanTypeCode());
	...
	String airbillNumber = state.getScan().getAirbillNumber();
	String[] locationList = new String[] {state.getScan().getLocationCode()};
	try {
	    handlingUnit = dbConnection.fetchHandlingUnit(state.getScan().getHandlingUnitOid());
	    //handlingUnit =  dbConnection.fetchHandlingUnit(state.getScan().getAirbillNumber(),locationList);
	}
	catch (Exception ex) {
